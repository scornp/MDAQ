...
class DisplayPanel extends JPanel implements Runnable{
  ...
  //  private SignalDisplaySettings signalDisplaySettings;
     ...
     */
/*   DisplayPanel(int width, int height, SignalDisplaySettings signalDisplaySettings, float [] dataArray) {
       this.width = width;
        this.height = height;
...
//        this.signalDisplaySettings = signalDisplaySettings;
   ...
   //     addMouseMotionListener(this);
    //    runa();
               ...
               DataPoint tailPoint = null;
               DataPoint headPoint = null;

    public void addData(float x){

        if (tailPoint == null){
            tailPoint = new DataPoint(x);
            headPoint = tailPoint;
        } else {
             dataPoint = new DataPoint(x);
            headPoint.setNextDataPoint(dataPoint);
             headPoint = dataPoint;
        }

        System.out.println("Attempting to set data element ----------------" + DataPoint.length);
        run();
    }
    ...
    public void run() {
        ...
        float x[];
        float y[];
        x = new float[DataPoint.length];
        y = new float[DataPoint.length];
        ...
        for (int i = 0; i < DataPoint.length; i++) {
            x[i] = (float) i;
            y[i] = dataPoint.getData();
            dataPoint = dataPoint.getNextDataPoint();
        }
        ...
        //      g.setStroke(stroke);
        for (j = 0; j < DataPoint.length - 1; j++) {
